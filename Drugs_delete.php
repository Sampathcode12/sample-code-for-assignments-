...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($searchTerm)) {
        $url = "http://localhost:5268/api/Drugs/DeleteDrugs/" . urlencode($searchTerm);
        ...
        if ($curlError) {
            ...
            if ($httpCode == 200 && isset($result['statusCode']) && $result['statusCode'] == 200) {
                ...
                $message = "No matching drug found or already deleted.";
            } else {
                $message = "An error occurred: " . ($result['statusMessage'] ?? "Unknown error.");
            }
        }
    } else {
        $message = "Please enter a Drug ID or Name.";
    }
}
?>
...
<body>
    ...
    <section class="delete-section">
        <form method="post">
            <input type="text" name="searchTerm" placeholder="Enter Drug ID or Name" required>
